 ...
 * @param {string} options.confirmText - 确认按钮文本（必填）
 * @param {string} options.cancelText - 取消按钮文本（必填）
 * @param {Function} options.validateFn - 异步验证函数 (value) => Promise<{ success: boolean, error?: string }>
 * @returns {Promise<string|null>} 验证通过时返回输入值，取消或关闭返回 null
 */
export async function showInputDialogWithValidation(options) {
  const {
    title,
    message,
    placeholder = '',
      ...
      if (typeof validateFn === 'function') {
        ...
        try {
          ...
          if (result && result.success) {
            ...
            showError((result && result.error) || 'Validation failed');
