...
async function resetWorkspaceSideEffects(workspaceRoot: string): Promise<void> {
  ...
  await knowledgeCopilotIndexService.shutdown().catch((error) => {
    logger.warn(`Failed to shutdown knowledge-copilot index service before lancedb cleanup: ${getErrorMessage(error)}`);
  });
