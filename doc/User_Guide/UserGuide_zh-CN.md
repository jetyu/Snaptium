...
**语言**: [简体中文](./UserGuide_zh-CN.md) | [English](./UserGuide_en-US.md)
...
NoteWizard 支持完整的笔记迁移和备份功能，确保您的数据始终可控。

**导入笔记:**
- 文件 → 打开 (或 `Ctrl+O`)
- 选择 `.md` 文件加载到编辑器。
- 也可以通过文件菜单导入 `.nwp` (NoteWizard 专有包) 文件。

**导出笔记:**
- 文件 → 导出 (或 `Ctrl+E`)
- **Markdown (.md)**: 标准 Markdown 格式，方便在其他编辑器中使用。
- **NoteWizard Package (.nwp)**: 专有备份包格式，包含所有元数据和结构，适合在不同设备间迁移 NoteWizard 数据。

### AI 写作助手

NoteWizard 集成了可选的 AI 写作助手，助力您的内容创作。

**配置说明:**
1. 打开 **首选项** (`Ctrl+Shift+P`) → **AI 服务**。
2. 选择您的服务商（如 OpenAI、DeepSeek、阿里通义等）。
3. 输入 **API 密钥 (API Key)**。为了安全起见，密钥会以加密形式存储在本地。
4. (可选) 定制 **系统提示词 (System Prompt)** 和 **API 接口地址 (Endpoint)**。

**功能使用:**
- 在编辑器中选中文字，即可获取针对性的写作建议。
- 点击工具栏的 AI 按钮，进行内容润色、总结或续写。
- AI 功能 **默认关闭**。只有在您主动操作时，相关内容才会发送至您配置的服务商。

### 数据安全与本地加密

安全是 NoteWizard 的基石。我们为敏感笔记提供工业级强度的本地加密保护。

**本地加密原理:**
- 应用采用 **AES-256-GCM** 算法对笔记进行设备端加密。
- 开启后，您的笔记数据在硬盘上将以加密密文形式存在，黑客即便拿走文件也无法读取。

**恢复密钥 (Recovery Key):**
- 首次开启加密时，系统会为您生成唯一的 **恢复密钥**。
- **重要提示**: 请务必将此密钥保存在安全的地方（如密码管理器中或打印出来）。
- 我们不存储您的密码或恢复密钥。如果您同时丢失了密码和密钥，加密数据将**永远无法恢复**。
