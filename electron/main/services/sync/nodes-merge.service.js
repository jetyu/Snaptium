...
function mergePresentNodes(localNode, remoteNode, baseNode) {
  const merged = {};
  const keys = new Set([
...
function resolveNode(localNode, remoteNode, baseNode) {
  if (!baseNode) {
    if (!localNode) {
...
export function mergeNodesJsonl({ baseText = '', localText = '', remoteText = '' }) {
