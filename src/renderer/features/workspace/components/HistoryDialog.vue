      ...
      <div v-if="workspaceStore.isHistoryDialogOpen" class="history-overlay" @keydown.esc="closeDialog" tabindex="0"
        ...
        <div class="history-modal" @click.stop>
          ...
          <div class="history-footer">
            <button class="btn-cancel" @click="closeDialog">{{ $t('dialog.cancel') }}</button>
...
const selectVersion = async (filename: string) => {
  ...
  selectedContentHtml.value = '';
  try {
    const rawContent = await workspaceStore.getHistoryContent(filename);
    selectedContentHtml.value = renderMarkdown(rawContent);
  } finally {
